...
async function generateSwissCalendars() {
    console.log('\n📅 Generating Swiss Holiday Calendars...');
    console.log('='.repeat(60));

    const currentYear = new Date().getFullYear();

    // Generate national holidays (all 4 national holidays)
    await IcsGenerator.generateRollingCalendar(
        getSwissNationalHolidays(),
        {
            name: 'Swiss National Holidays',
            productId: COMMON_APPLICATION_PRODID,
            calendarName: 'Schweizer Bundesfeiertage',
            description: 'National holidays observed in all 26 cantons',
            country: SWITZERLAND_COUNTRY_CODE
        },
        join(OUTPUT_DIR, 'swiss_holidays_national.ics'),
        currentYear
    );

    // Generate cantonal holidays
    for (const canton of swissCantons) {
        const holidays = getSwissHolidaysForCanton(canton);

        await IcsGenerator.generateRollingCalendar(
            holidays,
            {
                name: `Swiss Public Holidays - ${canton}`,
                productId: COMMON_APPLICATION_PRODID,
                calendarName: `Schweizer Feiertage - ${canton}`,
                country: SWITZERLAND_COUNTRY_CODE
            },
            join(OUTPUT_DIR, `swiss_holidays_${stateToFilename(canton)}.ics`),
            currentYear
        );
    }

    console.log(`\n✓ Swiss calendars generated for ${swissCantons.length} cantons + national`);
}
